...
using an AI coach (Google Gemini) that nudges rather than answers, and then
checks the user's final guess against real-world numbers pulled from Tavily
web search.
...
MODEL_OPTIONS = ["gemini-2.5-flash", "gemini-2.5-flash-lite", "gemini-3.5-flash"]
...
CONCLUSION_SYSTEM_PROMPT = """You are writing the closing summary of a person's Fermi-estimation \
walkthrough. You will be given the original question, their own notes for each of the 5 steps \
(Clarify, Structure, Estimate, Calculate, Sanity-Check), their final numeric guess, and -- if \
available -- a verdict on how that guess compared to a real-world reference number.

Write a short conclusion (4-6 sentences, flowing prose, NOT bullet points) that:
- Weaves together the throughline of THEIR OWN reasoning across the steps, in their own logic \
and their own words/assumptions -- not generic advice or a restatement of the framework.
- Names the specific choices or assumptions they actually made, where relevant, rather than \
speaking abstractly.
- If a verdict is available, folds it naturally into the story of how they got to their answer.
- Ends on one encouraging, forward-looking sentence.

Do not introduce new numeric facts or corrections. Do not lecture or re-teach the framework. \
Write in second person ("You started by...", "From there, you..."). Respond with plain prose only, \
...
def generate_conclusion(
    ...
    lines = [f"Guesstimate question: {question}", ""]
    for s in STEPS:
        answer = step_answers.get(s["key"], "").strip()
        lines.append(f"[{s['title']}] {answer or '(left blank)'}")
    lines.append("")
    lines.append(f"Final numeric guess: {final_guess or '(no guess given)'}")

    if verification:
        lines.append("")
        lines.append(
            f"Verdict on the guess: {verification.get('verdict', 'UNKNOWN')} "
            f"(reference estimate: {verification.get('reference_estimate', 'unknown')}, "
            f"{verification.get('ratio_note', '')})"
        )
    ...
    response = client.models.generate_content(
        ...
        contents=user_message,
        config=types.GenerateContentConfig(
            system_instruction=CONCLUSION_SYSTEM_PROMPT,
...
with st.sidebar:
    ...
    if gemini_key:
        ...
        st.caption(
            "Free to get at [aistudio.google.com](https://aistudio.google.com/app/apikey) "
...
if step_idx < len(STEPS):
        ...
        if st.button("Get a hint", key=f"hint_{step['key']}"):
            if not gemini_key:
                ...
                with st.spinner("Thinking..."):
                    hint = get_hint(gemini_key, model, st.session_state.question, step_idx)
    ...
    with st.container(border=True):
            ...
            if not gemini_key:
                ...
                with st.spinner("Searching and grading..."):
                    try:
                        ...
                        snippets = []
                    result = verify_guess(
                        gemini_key, model, st.session_state.question,
        ...
        st.markdown("---")
        st.markdown('<div class="eyebrow">Wrap-up</div>', unsafe_allow_html=True)
        st.subheader("Your Complete Thought Process")

        for s in STEPS:
            st.markdown(f"**{s['title']}** &middot; _{s['book']}_")
            st.write(st.session_state.step_answers[s["key"]] or "_(left blank)_")
        st.markdown(f"**Final Guess:** {st.session_state.final_guess or '_(none given)_'}")

        if st.button("Write my conclusion", type="secondary"):
            if not gemini_key:
                st.error("Add your Gemini API key in the sidebar first.")
            else:
                with st.spinner("Synthesizing your reasoning..."):
                    verification_for_conclusion = (
                        st.session_state.verification_result["result"]
                        if st.session_state.verification_result
                        else None
                    )
                    st.session_state.conclusion = generate_conclusion(
                        gemini_key, model, st.session_state.question,
                        st.session_state.step_answers, st.session_state.final_guess,
                        verification_for_conclusion,
                    )
