...
PROVIDER_MODELS = {
    "Gemini (free)": ["gemini-2.5-flash", "gemini-2.5-flash-lite"],
    "Anthropic (paid)": ["claude-sonnet-5", "claude-opus-4-8", "claude-haiku-4-5-20251001"],
}
...
def _call_anthropic(system_prompt: str, user_message: str, api_key: str, model: str, max_tokens: int) -> str:
...
def _call_gemini(system_prompt: str, user_message: str, api_key: str, model: str, max_tokens: int) -> str:
...
def call_ai(provider: str, system_prompt: str, user_message: str, api_key: str, model: str, max_tokens: int) -> str:
    if provider.startswith("Gemini"):
        return _call_gemini(system_prompt, user_message, api_key, model, max_tokens)
    return _call_anthropic(system_prompt, user_message, api_key, model, max_tokens)


def get_hint(provider: str, api_key: str, model: str, question: str, step_index: int) -> str:
    ...
    user_message = "\n".join(context_lines)
    return call_ai(provider, COACH_SYSTEM_PROMPT, user_message, api_key, model, max_tokens=550)


def verify_guess(provider: str, api_key: str, model: str, question: str, guess: str, snippets: list) -> dict:
    snippet_text = "\n\n".join(
    ...
    raw = call_ai(provider, VERIFY_SYSTEM_PROMPT, user_message, api_key, model, max_tokens=700)
...
with st.sidebar:
    ...
    provider = st.radio("AI provider", list(PROVIDER_MODELS.keys()), index=0)
    secret_name = "GEMINI_API_KEY" if provider.startswith("Gemini") else "ANTHROPIC_API_KEY"
    ai_key = get_secret(secret_name)
    if ai_key:
        st.success(f"{provider.split(' ')[0]} key loaded from secrets.toml")
    else:
        ai_key = st.text_input(f"{provider.split(' ')[0]} API key", type="password")
        if provider.startswith("Gemini"):
...
if step_idx < len(STEPS):
    ...
    with st.container(border=True):
        ...
        if st.button("Get a hint", key=f"hint_{step['key']}"):
            if not ai_key:
                st.error(f"Add your {provider.split(' ')[0]} API key in the sidebar first.")
            else:
                with st.spinner("Thinking..."):
                    hint = get_hint(provider, ai_key, model, st.session_state.question, step_idx)
            ...
            if not ai_key:
                ...
                with st.spinner("Searching and grading..."):
                    try:
                        ...
                        snippets = []
                    result = verify_guess(
                        provider, ai_key, model, st.session_state.question,
