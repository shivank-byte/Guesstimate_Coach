...
user's final guess against real-world numbers pulled from Google Custom Search.
...
def google_search(query: str, api_key: str, cx: str, num: int = 5) -> list:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("Google Verification")
    google_key = st.text_input("Google API key", type="password")
    google_cx = st.text_input("Google Custom Search Engine ID (cx)")
    st.caption(
        "Needed only for the final 'check my guess' step, via the Google "
...
if step_idx < len(STEPS):
    ...
    with st.container(border=True):
        ...
        if st.button("Check my guess", type="primary", disabled=not st.session_state.final_guess.strip()):
            if not anthropic_key:
                st.error("Add your Anthropic API key in the sidebar first.")
            elif not (google_key and google_cx):
                st.error("Add your Google API key and Search Engine ID (cx) in the sidebar first.")
            else:
                with st.spinner("Searching and grading..."):
                    try:
                        snippets = google_search(
